...
local leverInfo = {
	...
	[1] = {byDay = "Monday", bossName = "Alptramun", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.alptramunTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.alptramunTimer},

	[2] = {byDay = "Tuesday", bossName = "Izcandar the Banished", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.izcandarTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.izcandarTimer},

	[3] = {byDay = "Wednesday", bossName = "Malofur Mangrinder", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.malofurTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.malofurTimer},

	[4] = {byDay = "Thursday", bossName = "Maxxenius", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.maxxeniusTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.maxxeniusTimer},
	
	[5] = {byDay = "Friday", bossName = "Izcandar the Banished", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.izcandarTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.izcandarTimer},

	[6] = {byDay = "Saturday", bossName = "Plagueroot", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.plaguerootTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.plaguerootTimer},

	[7] = {byDay = "Sunday", bossName = "Maxxenius", bossPosition = Position(32208, 32048, 14), leverPosition = Position(32208, 32020, 13),
	pushPosition = Position(32208, 32021, 13), leverFromPos = Position(32208, 32021, 13), leverToPos = Position(32208, 32025, 13),
	storageTimer = Storage.DreamCourts.DreamScar.maxxeniusTimer, roomFromPosition = Position(32198, 32037, 14),
	roomToPosition = Position(32234, 32054, 14), teleportTo = Position(32224, 32048, 14), typePush = "y", exitPosition = Position(32208, 32035, 13),
	globalTimer = GlobalStorage.DreamCourts.DreamScar.maxxeniusTimer},
...
local lastBoss = {
	...
	storageTimer = Storage.DreamCourts.DreamScar.nightmareTimer,
	globalTimer = GlobalStorage.DreamCourts.DreamScar.nightmareTimer,
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 10029 then
		if iPos == nightmareLever then
			if doCheckBossRoom(player:getId(), lastBoss.bossName, lastBoss.roomFromPosition, lastBoss.roomToPosition) then		
					...
					for x = lastBoss.fromPositionTeleport.x, lastBoss.toPositionTeleport.x do
						for y = lastBoss.fromPositionTeleport.y, lastBoss.toPositionTeleport.y do
							...
							if c and c:isPlayer() then
								...
								c:setStorageValue(lastBoss.storageTimer, os.stime() + 20*60*60)							
							end
						end
					end
					local monster = Game.createMonster(lastBoss.bossName, lastBoss.bossPosition)
					if monster then
							...
							if leverTable.typePush == "x" then
								for i = leverTable.leverFromPos.x, leverTable.leverToPos.x do
									...
									if creature and creature:isPlayer() then
										...
										creature:setStorageValue(leverTable.storageTimer, os.stime() + 20*60*60)
							...
							if monster then
								if leverTable.bossName:lower() == "maxxenius" then							
									...
									for i = 1, #generators do				
										Game.createMonster("Generator", generators[i])	
									end	
									monster:registerEvent("facelessHealth")
								elseif leverTable.bossName:lower() == "alptramun" then 	
									for i = 1, 2 do
										local summon = Game.createMonster("unpleasant dream", leverTable.bossPosition, false, false)
								...
								elseif leverTable.bossName:lower() == "malofur mangrinder" then
									for i = 1, #whirlingBlades do
										local blade = Game.createMonster("whirling blades", whirlingBlades[i])
									end
								end
								monster:registerEvent("dreamCourtsDeath")
